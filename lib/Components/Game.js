...
function renderTilePreview(x, y, text, player, dim) {
    ...
    var tile = getTileFromOwner(text, player);
...
function renderBoard(board) {
    ...
    for (var x = 0; x < board.BOARD_WIDTH; x++) {
        for (var y = 0; y < board.BOARD_HEIGHT; y++) {
    ...
    if (closeUpScreen) {
        ...
        var xoffset = (canvas.width - canvas.height) / 2;
...
function renderBoardPreview(board, ix, iy, height) {
    ...
    var tileDim = height / board.BOARD_WIDTH;
...
function renderPlayerList() {
    ...
    var maxBoardSize = 25 * 5;
    var xPadding = 300;
    var yPadding = 0;
    var textYPadding = 45;
    var h = canvas.height / Session.PLAYERS.length;
    ...
    for (var p = 0; p < Session.PLAYERS.length; p++) {
        ...
        if (p > 0) {
            if (p % 2 == 0) {
                ...
                var player = Session.PLAYERS[p];

                var board = getBoardByOwner(player);
                ...
                var _player = Session.PLAYERS[p];

                var _board = getBoardByOwner(_player);

                renderBoardPreview(_board, canvas.width - h - 15, yo * h, h - 10);
                yo += 1;
            }
        } else {
            context.fillStyle = "rgb(175,175,175)";
            ...
            var _player2 = Session.PLAYERS[p];

            var _board2 = getBoardByOwner(_player2);

            renderBoardPreview(_board2, canvas.width / 2 - h - 15, yo * h, h - 10);
        }
    }
...
function getTile(text) {
    ...
    var board = getBoardByOwner(User);
...
function checkHoverOverTiles(mx, my) {
    ...
    for (var x = 0; x < board.BOARD_WIDTH; x++) {
        for (var y = 0; y < board.BOARD_HEIGHT; y++) {
            ...
            var tile = getTileByXY(x, y);

            if (mx >= x * TILE_WIDTH && mx <= x * TILE_WIDTH + TILE_WIDTH && my >= y * TILE_HEIGHT && my <= y * TILE_HEIGHT + TILE_HEIGHT) {
...
function clickButton(mx, my) {
    ...
    if (mx >= xoffset && mx <= xoffset + canvas.height / 2 && my >= canvas.height - 200 && my <= canvas.height) {
        selectedTile.CROSSED = !selectedTile.CROSSED;
        selectedTile = {};
        closeUpScreen = false;
    } else if (mx >= xoffset + canvas.height / 2 && mx <= xoffset + canvas.height && my >= canvas.height - 200 && my <= canvas.height) {
        selectedTile = {};
        closeUpScreen = false;
    }
...
function clickEndButton(mx, my) {
    ...
    if (mx >= button_restartGame.x && mx <= button_restartGame.x + button_restartGame.width && my >= button_restartGame.y && my <= button_restartGame.y + button_restartGame.height) {

        if (Session.PLAYER_HOST.USERNAME == User.USERNAME) {
            button_restartGame.clicked();
        }
    } else if (mx >= button_disconnectGame.x && mx <= button_disconnectGame.x + button_disconnectGame.width && my >= button_disconnectGame.y && my <= button_disconnectGame.y + button_disconnectGame.height) {
